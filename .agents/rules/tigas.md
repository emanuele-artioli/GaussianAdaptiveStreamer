...
description: When working on tigas: Modular 3DGS/4DGS remote-rendering research testbed: server-side Gaussian Splatting renders a scene, streams it to a client over an ABR-controlled transport, and an offline evaluation component sweeps quality/bitrate tradeoffs. Six runtime stages (per `README.md` / `docs/ARCHITECTURE.md`): input/con
---
